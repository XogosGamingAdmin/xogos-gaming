...
class Chat
{
	...
	public function chatUsers($userid)
	{
		...
		// var_dump($results[0]["user_role"]);die;
		...
		foreach ($results as $result) {
			...
			if ($result["user_role"] == "student") {
				...
				if (!empty($result["class_id"])) {
					...
					//echo $sqlQuery;
					$sqlQuery = "SELECT b.* FROM friends_list a left join ".$this->chatUsersTable." b ON CASE WHEN a.user_id = '" . $result["user_id"] . "' THEN a.friend_id = b.user_id WHEN a.friend_id = '". $result["user_id"] ."' THEN a.user_id = b.user_id  END where (a.user_id = '" . $result["user_id"] . "' or a.friend_id = '". $result["user_id"] ."')  AND a.status = 1";
					//	die();
					//	$tech_results = $this->getData($sqlQuery);
